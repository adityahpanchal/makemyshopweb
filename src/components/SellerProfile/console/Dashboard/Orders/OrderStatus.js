...
const OrderStatus = (props) =>{
    ...
    useEffect(() => {
        fetch(`${API}/get/order/status/seller/${props.match.params.buyerId}/${props.match.params.transactionId}/${userState.user._id}`, {
            method: "GET",
            headers: {
            ...
            if(data.error){
               ...
               if(data.orders){
                    ...
                    setOrderStatus(data.orders[0].orderStatus)
    ...
    const updateStatus = () =>{
        setWait(true)
        fetch(`${API}/update/order/delivery/${props.match.params.buyerId}/${props.match.params.transactionId}/${userState.user._id}/${orderStatus}`, {
            method: "POST",
            headers: {
                Accept: "application/json",
                Authorization: `Bearer ${userState.token}`
                ...
                <main className="col-md-12">
                    <article className=" mb-4">
                        ...
                        <div className="row">
                            ...
                            {!isUpdate && <button onClick={() => setIsUpdate(true)} className="btn btn-outline-warning mx-3 my-3">Update Status</button>}
                            {
                                isUpdate &&
                                <>
                                <select onChange={(e) => setOrderStatus(e.target.value)} className="form-control col-4 my-3 ">
                                ...
                                <button onClick={updateStatus} className="btn btn-success mx-3 my-3">{wait ? 'Plz wait a sec' : 'Save'}</button>
                                {!wait && <button onClick={() => setIsUpdate(false)} className="btn btn-danger my-3">Cancel</button>}
                                </>
                            }
                        </div>
