...
const ProductThree = ({index, setIndex, setData, modalType, dataForUpdate}) =>{
    ...
    const multiType = () =>{
        ...
        const saveEdit = (id) =>{
            let onearr = arr.filter((x) => x.id === id)[0]
            console.log(onearr)
            if(tval.price !== "" && tval.mrp !== "" && tval.price > tval.mrp){
                setExistedErr({isErr: true, err: 'price should be r less than mrp'})
                settval({key: '', value: '', mrp: '', price: ''})
                setTimeout(() => {
                    setExistedErr({isErr: false, err: ''})
                }, 800)
            ...
            if(tval.price === "" && onearr.price > tval.mrp){
                setExistedErr({isErr: true, err: 'price should be r less than mrp'})
                settval({key: '', value: '', mrp: '', price: ''})
                setTimeout(() => {
                    setExistedErr({isErr: false, err: ''})
            ...
            if(tval.mrp === "" && onearr.mrp < tval.price){
                setExistedErr({isErr: true, err: 'price should be less than mrp'})
                settval({key: '', value: '', mrp: '', price: ''})
                setTimeout(() => {
                    setExistedErr({isErr: false, err: ''})
                    ...
                    <table class="table col-9 mx-auto">
                        <thead>
                            {arr && arr.map((row) =>
                                row.isEditing ? 
                                <tr>
                                    <td><input className="form-control" type="text" onChange={(e) => settval({...tval, key: e.target.value.toUpperCase()})} defaultValue={row.key} /></td>
