...
const SearchProductBar = () =>{
    ...
    useEffect(() =>{
        ...
        if(userState.isAuthenticated){
            apiPath = `${API}/search/products/name/india/${userState.user.state}/${userState.user.city}?q=${query.get("q")}`
        }else{
            apiPath = `${API}/search/products/name/india?q=${query.get("q")}`
        }
        setLoading(true)
        fetch(apiPath, {
            method: "GET",
                        ...
                        <ul className="row no-gutters bordered-cols ">
                            {
                                productList && productList.map((x) =>{
                                    ...
                                    return(
                                        <li className="col-12 col-lg-4 col-md-4 p-3">
